...
RUN pip install --upgrade --ignore-installed https://storage.googleapis.com/tensorflow/linux/gpu/tensorflow-0.9.0-cp27-none-linux_x86_64.whl
RUN pip install --upgrade --ignore-installed git+https://github.com/fchollet/keras.git
...
RUN cd /gr/ && pybombs install -u gnuradio gr-burst gr-pyqt gr-pcap gr-mapper gr-mediatools
RUN cd /gr/ && git clone https://github.com/peteIS/gr-analysis && cd gr-analysis && mkdir build && cd build 
RUN cd /gr/gr-analysis/build/ cmake ../ 
RUN cd /gr/gr-analysis/build/ make 
RUN cd /gr/gr-analysis/build/ make install 
RUN cd /gr/gr-analysis/build/ ldconfig
...
RUN cd /root/src/
RUN git clone https://github.com/PyOpenPNL/OpenPNL.git 
RUN cd OpenPNL && ./autogen.sh && ./configure && make -j4 && make install
